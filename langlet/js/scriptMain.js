...
class Aplikace {
  ...
  constructor() {
    ...
    /*this.#prihlasenyUzivatel.pocetOtazek = 0;
    ...
    this.#prihlasenyUzivatel.zaznamyTestu = [];
  ...
  pridatDvojiciSlov(e) {
    ...
    slovoPridanePreklad = slovoPridanePreklad.split(",");
    ...
    console.log(JSON.stringify(this.#prihlasenyUzivatel.slovicka));
    pridaneSlovoCizi.value = "";
    pridaneSlovoPreklad.value = "";
    pridaneSlovoCizi.focus();
    this.generovatPrehledSoucasnyKrok(e);
  ...
  odebratDvojiciSlov(e) {
    let odebranadvojice = this.#prihlasenyUzivatel.slovicka[this.#prihlasenyUzivatel.prohlizenyJazyk][this.#prihlasenyUzivatel.prohlizenaLekce].find(function (dvojice) {
      return dvojice.includes(e.target.closest("a").textContent)
    });
  ...
  async nacistData() {
    ...
    await schovanyKontejner3.setAttribute("value", data);
    await btnAktualizovat.click();
  }
  ...
  generovaniSibenice(e) {
    ...
    let napoveda = this.#prihlasenyUzivatel.sibenicePismena.length - 2;
    //let napoveda = Math.floor(this.#prihlasenyUzivatel.sibenicePismena / 2) ? Math.flat(this.#prihlasenyUzivatel.sibenicePismena / 2) : 1;
    let pismenoNapoveda = " ";
    while (napoveda > 1) {
  ...
  generovaniVyberPrekladu(e) {
    if (e) e.preventDefault();
    if (this.#prihlasenyUzivatel.slovicka[this.#prihlasenyUzivatel.soucasnyJazyk][this.#prihlasenyUzivatel.soucasnaLekce].length < 2) {
      return;
    }
    let trojiceSlov
    let prekladanaDvojice
  ...
  kontrolaJazyk1Jazyk2(e) {
    e.preventDefault();
    if (!this.#prihlasenyUzivatel.prekladaneSlovoHledane1) {
      return
    }
  ...
  kontrolaJazyk2Jazyk1(e) {
    e.preventDefault();
    if (!this.#prihlasenyUzivatel.prekladaneSlovoHledane2) {
      return
    }
  ...
  kontrolaSibenice(e) {
    e.preventDefault();
    if (!this.#prihlasenyUzivatel.slovoSibenice) {
      return
    }
  ...
  kontrolaVyberPrekladu(e) {
    e.preventDefault();
    if (!this.#prihlasenyUzivatel.vyberPrekladuSpravne) {
      return
    }
    let vybraneSlovo;
    try {
